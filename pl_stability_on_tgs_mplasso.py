...
RSCRIPT_EXE = r"C:/PROGRA~1/R/R-43~1.3/bin/x64/Rscript.exe"
R_SCRIPT = r"C:/Users/enthe/Desktop/Thesis/r_code_fs_and_bt/fit_and_select_cli.r"
...
def cpss_stability_selection_tgs(X, y, z_idx, B_pairs=50, cv=0.1, seed=123):
    """
    CPSS stability selection for TGS data, using an external R script
    (fit_and_select_cli.r) to compute mains/interactions on each half-sample.

    For now, fit_and_select_cli.r is a test script that always returns all FALSE.
    Once this works end-to-end, you can switch that script to call plasso_fit1.
    """
    ...
    def fit_and_select(sub_idx: np.ndarray):
        """
        Call R's fit_and_select_cli.r on a subset of rows.

        Returns
        -------
        mains_sel : bool array, shape (p,)
        inter_sel : bool array, shape (p, p)
        """
        Xb = X[sub_idx]
        yb = y[sub_idx]
        Zb = Z_full[sub_idx]
        #do we save everytime a different file A,B??

        with tempfile.TemporaryDirectory() as tmpdir:
            X_path = os.path.join(tmpdir, "X.csv")
            y_path = os.path.join(tmpdir, "y.csv")
            Z_path = os.path.join(tmpdir, "Z.csv")
            mains_out_path = os.path.join(tmpdir, "mains_sel.csv")
            inter_out_path = os.path.join(tmpdir, "inter_sel.csv")

            # Save sub-sample to disk
            np.savetxt(X_path, Xb, delimiter=",")
            np.savetxt(y_path, yb, delimiter=",")
            np.savetxt(Z_path, Zb, delimiter=",")
            ...
            try:
                ...
                raise RuntimeError(
                    "R script fit_and_select_cli.r failed.\n"
                    f"Command: {' '.join(cmd)}\n"
            ...
            if result.stdout.strip():
                print("R stdout:", result.stdout.strip())
            if result.stderr.strip():
                print("R stderr:", result.stderr.strip())

            mains_sel = np.loadtxt(mains_out_path, delimiter=",")
            mains_sel = np.atleast_1d(mains_sel).astype(bool)

            inter_sel_pmod = np.loadtxt(inter_out_path, delimiter=",")
            inter_sel_pmod = np.atleast_2d(inter_sel_pmod).astype(bool)

        # sanity checks
        if mains_sel.size != p:
            raise ValueError(f"R returned mains_sel of size {mains_sel.size}, expected {p}")

        if inter_sel_pmod.size != p * p_mod:
            raise ValueError(
                f"R returned inter_sel of size {inter_sel_pmod.size}, expected {p * p_mod}"
            )

        inter_sel_pmod = inter_sel_pmod.reshape(p, p_mod)

        # Map (p x p_mod) -> (p x p) using z_idx
        inter_sel = np.zeros((p, p), dtype=bool)
        for i in range(p):
            for jj in range(p_mod):
                if inter_sel_pmod[i, jj]:
                    j = int(z_idx[jj])
                    inter_sel[i, j] = True
                    inter_sel[j, i] = True
    ...
    for b in range(B_pairs):
        ...
        if np.unique(y[A]).size < 2 or np.unique(y[B]).size < 2:
            print(f"[CPSS pair {b}] Skipping: one half has only one class.")
            continue

        print(f"[CPSS pair {b}] Fitting on halves of sizes {A.size} and {B.size}...")
    ...
    if K_eff == 0:
        raise RuntimeError("All CPSS pairs failed; no valid splits.")
...
def main():
    ...
    for data_path, balance_tag in data_files:
        ...
        # Save results
        pmf_mains = empirical_pmf(K_mains_halves)
        pmf_inters = empirical_pmf(K_inter_halves)
        q_mains = pmf_mains["q"]
        q_inters = pmf_inters["q"]
        q_total = q_mains + q_inters

        # Optionally print or save:
        print(f"q_mains: {q_mains:.2f}, q_inters: {q_inters:.2f}, q_total: {q_total:.2f}")
        ...
        K_total_halves = [a + b for a, b in zip(K_mains_halves, K_inter_halves)]
        ...
        q_json_path = os.path.join(CONFIG["RESULTS_DIR"], f"q_stats_{balance_tag}.json")
        with open(q_json_path, "w") as f:
            json.dump(q_json, f, indent=2)
        print(f"Saved q values JSON → {q_json_path}")
